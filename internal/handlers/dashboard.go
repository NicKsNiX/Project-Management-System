...
type masterPlanSummaryRow struct {
	IpID           int64            `db:"ip_id"`
	IpCode         utils.NullString `db:"ip_code"`
	IpCustomerName utils.NullString `db:"ip_customer_name"`
	IpModel        utils.NullString `db:"ip_model"`
	IpPartNo       utils.NullString `db:"ip_part_no"`
	IpPartName     utils.NullString `db:"ip_part_name"`
	IpSopDate      *time.Time       `db:"ip_sop_date"`
	MmpOrderNo     int64            `db:"mmp_order_no"`
	MilestoneName  string           `db:"milestone_name"`
	IpmpID         *int64           `db:"ipmp_id"`
	IpmpStatus     utils.NullString `db:"ipmp_status"`
	IpmpStartDate  *time.Time       `db:"ipmp_start_date"`
	IpmpEndDate    *time.Time       `db:"ipmp_end_date"`
}
...
func ListMasterPlanSummary(c *fiber.Ctx, db *sqlx.DB) error {
	query := `SELECT
					...
					ip.ip_sop_date,
					mmp.mmp_order_no,
					mmp.mmp_name AS milestone_name,
					ipmp.ipmp_id,
					ipmp.ipmp_status,
					ipmp.ipmp_start_date,
					ipmp.ipmp_end_date
				FROM info_project ip
				CROSS JOIN mst_master_plan mmp
				LEFT JOIN info_project_master_plan ipmp
					ON ipmp.ip_id = ip.ip_id
					AND ipmp.ipmp_name = mmp.mmp_name
				...
				ORDER BY
					...
					ipmp.ipmp_id DESC,
					mmp.mmp_id;`
	var rows []masterPlanSummaryRow
	if err := db.Select(&rows, query); err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "query error", "detail": err.Error()})
	}

	summaries := make([]MasterPlanSummary, 0)
	projectIndex := make(map[int64]int)
	seenMilestones := make(map[int64]map[int64]bool)

	for _, row := range rows {
		idx, exists := projectIndex[row.IpID]
		if !exists {
			summaries = append(summaries, MasterPlanSummary{
				IpID:           row.IpID,
				IpCode:         row.IpCode,
				IpCustomerName: row.IpCustomerName,
				IpModel:        row.IpModel,
				IpPartNo:       row.IpPartNo,
				IpPartName:     row.IpPartName,
				IpSopDate:      row.IpSopDate,
				Milestones:     make([]MasterPlanMilestone, 0),
			})
			idx = len(summaries) - 1
			projectIndex[row.IpID] = idx
			seenMilestones[row.IpID] = make(map[int64]bool)
		}

		if seenMilestones[row.IpID][row.MmpOrderNo] {
			continue
		}
		seenMilestones[row.IpID][row.MmpOrderNo] = true

		summaries[idx].Milestones = append(summaries[idx].Milestones, MasterPlanMilestone{
			Name:      row.MilestoneName,
			Status:    row.IpmpStatus,
			StartDate: row.IpmpStartDate,
			EndDate:   row.IpmpEndDate,
		})
	}
